...
const ATTR = {
  ...
  fetchpriority: 'fetchPriority', imagesrcset: 'imageSrcSet', imagesizes: 'imageSizes',
...
const ENT = {
  ...
  '&check;': '\u2713', '&cross;': '\u2717', '&nearr;': '\u2197', '&searr;': '\u2198',
  '&swarr;': '\u2199', '&nwarr;': '\u2196', '&uarr;': '\u2191', '&darr;': '\u2193',
  '&harr;': '\u2194', '&lsaquo;': '\u2039', '&rsaquo;': '\u203a',
  '&ldquo;': '\u201c', '&rdquo;': '\u201d', '&bull;': '\u2022', '&dagger;': '\u2020',
  '&laquo;': '\u00ab', '&raquo;': '\u00bb', '&sect;': '\u00a7', '&para;': '\u00b6',
  '&reg;': '\u00ae', '&trade;': '\u2122', '&frac12;': '\u00bd', '&plusmn;': '\u00b1',
  '&ne;': '\u2260', '&le;': '\u2264', '&ge;': '\u2265', '&infin;': '\u221e',
  '&prime;': '\u2032', '&Prime;': '\u2033', '&emsp;': '\u2003', '&ensp;': '\u2002',
  '&thinsp;': '\u2009', '&shy;': '\u00ad', '&zwj;': '\u200d', '&zwnj;': '\u200c',
};

/* An entity that survives into JSX text renders as its literal source ("&check;"),
   which is a silent, page-breaking bug \u2014 so an unknown one is a hard failure. */
function decode(v) {
  return String(v).replace(/&#(\d+);|&#x([0-9a-fA-F]+);|&[a-zA-Z][a-zA-Z0-9]*;/g, (m, dec, hex) => {
    if (dec) return String.fromCodePoint(+dec);
    if (hex) return String.fromCodePoint(parseInt(hex, 16));
    if (ENT[m] !== undefined) return ENT[m];
    throw new Error('dc2next: unknown HTML entity ' + m + ' \u2014 add it to ENT in tools/dc2next.mjs');
  });
}
...
/* Entities are decoded here rather than left to the JSX transform: SWC's named-entity
   table is narrower than HTML's — it passes &check; straight through as literal text —
   so the only safe thing to emit is the character itself. Decode first, then escape:
   an &lt; in the copy has to end up as {'<'}, exactly like a bare < does. */
function textToJsx(txt) {
  if (!txt.trim()) return txt.replace(/[{}<>]/g, '');
  const marked = decode(txt.replace(HOLE, (m, n) => '\u0000' + holeExpr(n) + '\u0001'));
...
export function convert(src, { component, prefix = '' }) {
  ...
  /* the sources use paths relative to the design folder; those resolve at "/" but 404
     from any nested route in a static export, so every asset ref becomes absolute */
  body = body.replace(/(src|href)="(?:\.\/)?(photos\/)/g, '$1="/$2');
  body = body.replace(/url\((&quot;|"|')?(?:\.\/)?(photos\/)/g, 'url($1/$2');
