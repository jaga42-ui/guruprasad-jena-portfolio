...
class Page extends React.Component {
  ...
  plotBand(s) {
    ...
    const fig = [].slice.call(s.querySelectorAll("[data-arch2],[data-arch3],[data-arch4]")).filter((f) => f.style.display !== "none")[0];
  ...
  plotFigure(s, fig) {
    ...
    const trace = (el, ms, dur) => {
      if (!el) return;
      el.style.transition = "stroke-dashoffset " + dur + "ms cubic-bezier(.32,.72,.28,1) " + ms + "ms";
    ...
    let t = 240;
    const frames = list("[data-frame]");
    frames.forEach((f, i) => {
      const ms = t + i * 120;
      trace(f.querySelector("[data-line]"), ms, 720);
      show(list("text", f), ms + 380);
    });
    t += frames.length * 120 + 420;
    const plates = list("[data-plate]");
    plates.forEach((pl, i) => {
      const ms = t + i * 118;
      trace(pl.querySelector("[data-line]"), ms, 480);
      show(list("text", pl), ms + 230);
    });
    t += plates.length * 118 + 280;
    const wires = list("[data-wire]");
    wires.forEach((w, i) => {
      const ms = t + i * 78;
      trace(w.querySelector("[data-line]"), ms, 380);
      show(list("[data-arrow]", w), ms + 300);
      show(list("text", w), ms + 340);
    });
    const total = t + wires.length * 78 + 1000;
    clearTimeout(s.__settle);
  ...
  fill(p) {
    ...
    const nm = p.name || "";
    const own = /getfreetools/i.test(nm) ? arch3 : /sahayam/i.test(nm) ? arch4 : /umbrix/i.test(nm) ? arch2 : null;
    const two = !!own || p.nodes.length > 4;
    [arch2, arch3, arch4].forEach((f) => { if (f) f.style.display = f === own ? "" : "none"; });
    if (arch) arch.style.display = own ? "none" : "";
    const cap = this.root.querySelector("[data-figcap]");
    if (cap) cap.textContent = own === arch3 ? "FIG. 01 \u2014 CLIENT-SIDE COMPUTE PATH" : own === arch4 ? "FIG. 01 \u2014 ALERT & MATCH PATH" : own === arch2 ? "FIG. 01 \u2014 INGESTION & TAILORING PATH" : "FIG. 01 \u2014 SIGNAL PATH";
